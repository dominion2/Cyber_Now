🛠️ Lab Guide: Extracting and Applying the 256-Byte Mask

This guide provides the steps to reproduce the mask extraction process from a malware sample using Radare2 and a Python deobfuscator, based on forensic patterns identified during analysis.

🔍 Phase 1: Strategic Reasoning & Logic Discovery

Malware like DCRat uses a "Rolling XOR" to encrypt C2 traffic. To reproduce this, we must find the Data (the mask) and the Logic (how it's applied). Our strategy relied on Functional Convergence: using two independent paths to prove the same memory location was the key.
...
Assembly Breakdown of Loop Analysis (hit6_0):
...
def deobfuscate(data_path, mask_path, out_path):
    """
    ...
    for i in range(len(data)):
        ...
        mask_byte = mask[i % mask_len]
        # Replicates XOR deobfuscation
        decrypted.append(data[i] ^ mask_byte)
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python3 deobfuscate.py <encrypted_file> <mask_file> [output_file]")
    else:
